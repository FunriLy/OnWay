...
import qg.fangrui.boot.exception.HttpConnectException;
...
public class HttpUtils {
    ...
    public static String sendGet(String reqUrl, Map<String, String> params) throws URISyntaxException, IOException {
        ...
        try {
            ...
            LOGGER.debug("HTTP Get请求URL:{0} 请求结果:{1}", reqUrl, result);
            return result;
        } finally {
            if (inputStream != null){
                inputStream.close();
     ...
     */
    @SuppressWarnings("resource")
    public static String sendPost(String reqUrl, Map<String, String> params) throws HttpConnectException {
        try {
            ...
            for (String key : ketSet){
                ...
                if (list.size() > 0){
                    ...
                    try {
                        ...
                        LOGGER.debug("HTTP POST请求URL:{0} 请求结果:{1}", reqUrl, result);
                        return result;
                    } catch (Exception e) {
                        LOGGER.error("网络连接错误提示信息 : {0}", e.getMessage());
                        throw new Exception("HTTP POST请求 网络连接失败，请连接网络后重试!");
                    } finally {
                        inPut.close();
                    }
                } else {
                    LOGGER.error("HTTP POST请求 错误提示 : 参数不全");
                    throw new HttpConnectException("参数不全，请稍后重试!");
                }
            }
        } catch (Exception e){
            LOGGER.error("HTTP POST请求 未知错误提示 : {0}", e.getMessage());
            throw new HttpConnectException("发送未知异常");
        }
        return null;
    }
